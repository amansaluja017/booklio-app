...
export const createService = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!parsedData.success) {
      return res
        .status(400)
        .json({ error: (z as any).treeifyError(parsedData.error).properties });
    }

    const { name, category, description, images, price, status } =
      parsedData.data;
    ...
    try {
      ...
      const isValidProvider = await Provider.findOne({
      ...
      if (isValidProvider.address === null) {
        throw new ApiError(
          400,
          "Please setup your store address before adding services",
        );
      }
...
export const getAllServices = asyncHandler(
  async (_req: Request, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Internal server error: Failed to get all services" });
    }
  },
);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res
        ...
        .json(
          new ApiResponse(200, {
            message: "Provider services fetch successfully",
            services,
          }),
        );
    } catch (err) {
      ...
      return res
        .status(500)
        .json({
          error: "Internal server error: Failed to get provider services",
        });
    }
  },
);
...
export const updateServiceStatus = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!provider) {
      return res
        .status(401)
        .json({ error: "Unauthorized: Provider authentication required" });
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({
          error: "Internal server error: Failed to update service status",
        });
    }
  },
);

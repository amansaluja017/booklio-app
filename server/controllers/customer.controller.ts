  ...
  try {
    ...
    if (!newCustomer) {
      return res.status(500).json(
        new ApiResponse(500, null, "Failed to create customer")
      );
    }

    return res.status(201).json(
      new ApiResponse(201, { newCustomer }, "Customer registered successfully")
    );
  } catch (error) {
    console.error(error);
    return res.status(500).json(
      new ApiResponse(500, null, "An error occurred while registering the customer")
    );
  }
});
...
export const getCustomerProfile = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, customer, "User profile fetched successfully"));
});
...
export const updateCustomerAddress = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json(new ApiResponse(200, { address: updateAddressRes.address }, "address updated successfully"));
  } catch (error) {
    console.error(error);

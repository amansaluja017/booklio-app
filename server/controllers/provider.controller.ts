...
export const registerProvider = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!newProvider) {
        return res
          .status(500)
          .json(new ApiResponse(500, null, "Failed to create provider"));
      }
...
export const loginProvider = asyncHandler(
  async (req: Request, res: Response) => {
    const body = req.body;

    const parsedData = loginProviderValidation.safeParse(body);
    ...
    const user = await Provider.findOne({ email }).select(
...
export const logoutProvider = asyncHandler(
  async (req: Request, res: Response) => {
    const provider = req.provider;
...
export const updateProviderProfile = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!parseData.success) {
      return res
        .status(400)
        .json({ error: (z as any).treeifyError(parseData.error).properties });
    }

    const { phone, store, city, state, country, zipCode, description } =
      parseData.data;
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "Store set up successfully", updateProvider });
    } catch (error) {
      console.error(error);
...
export const updateProviderPassword = asyncHandler(
  async (req: Request, res: Response) => {
...
export const rejectProvider = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(200)
        .json(new ApiResponse(200, provider, "Provider rejected successfully"));
    } catch (error) {
      console.error(error);
      throw new ApiError(500, "Internal server error");
    }

...
export default function SetupStoreForm({
  setIsSetupCompletePanel,
}: {
  setIsSetupCompletePanel: (open: boolean) => void;
}) {
  ...
  const submit = async (data: LoginFormTypes) => {
    try {
      const response = await apiClient.setupStore(data);

      if (response) {
        dispatch(Update({ ...user.userData, store: data }));
        setIsSetupCompletePanel(false);
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/50 z-50 animate-fade-in">
      <Card className="w-full max-w-sm outline-none">
        <CardHeader>
          <CardTitle>Setup store</CardTitle>
          <CardDescription>
            Please provide the following details to setup your store.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(submit)}>
            ...
            <CardFooter className="flex-col gap-2">
              ...
              <Button
                onClick={() => setIsSetupCompletePanel(false)}
                type="submit"

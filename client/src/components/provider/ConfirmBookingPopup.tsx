...
import MarkCompleted from "./MarkCompletedForm";
...
export default function ConfirmBookingPopup({
  ...
  const [popupBtn, setPopupBtn] = useState<string>("");
  const [markCompletedPopup, setMarkCompletedPopup] = useState<boolean>(false);
  const [markInProgressPopup, setMarkInProgressPopup] =
    useState<boolean>(false);

  useEffect(() => {
    if (request.status) {
      if (request.status === "confirmed") setMarkInProgressPopup(true);
      else if (request.status === "in-progress") setMarkCompletedPopup(true);
      else if (request.status === "completed") setPopupBtn("Completed");
      else if (request.status === "cancelled") setPopupBtn("Cancelled");
    }
  }, [request.status]);

  const cancelBooking = async (status: string) => {
    try {
      await apiClient.cancelBooking({ bookingId: request._id, status });
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 lg:p-10 transform transition-all duration-300 ease-out scale-100 animate-slide-up">
        ...
        <div>
          <div className="mt-10">
            ...
            <p className="mt-2 p-2">{request?.notes || "No notes provided."}</p>
          </div>
        </div>
        ...
        {request.status !== "completed" && request.status !== "cancelled" && (
          <div className="flex items-center gap-3 mt-10 justify-end">
            {markCompletedPopup && (
              <MarkCompleted
                request={request}
                setMarkCompletedPopup={setMarkCompletedPopup}
              />
            )}
            ...
            {markInProgressPopup && (
              <MarkInProgress
                request={request}
                setMarkInProgressPopup={setMarkInProgressPopup}
              />
            )}

...
const getStatusColor = (status: data["status"]) => {
  const statusColors: Record<data["status"], string> = {
...
export function RequestCards() {
  const [request, setRequest] = useState<Array<data> | null>([]);
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await apiClient.getCustomerBookings();
        const data = response as {data: Array<data>};
            ...
            {request && request.length > 0 ?
              request.map((req, key) => (
                <TableRow key={key} className="hover:bg-gray-50 border-b">
                  ...
                  <TableCell className="italic">{req.otp}</TableCell>

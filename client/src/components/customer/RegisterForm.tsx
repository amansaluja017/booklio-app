...
import { customerRegister } from "../../../../server/validations/validation";
...
export function RegisterForm() {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm({ resolver: zodResolver(customerRegister) });
  ...
  const submit = async (data: object) => {
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : "Failed to register customer")
    }
  };
            ...
            <div className="flex flex-col gap-6">
              ...
              <p className="text-xs text-muted-foreground text-red-500">
                {(errors.email?.message as string) ||
                  (errors.password?.message as string) || errorMessage}
              </p>
